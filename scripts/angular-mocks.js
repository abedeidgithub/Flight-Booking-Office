 ...
 * @param {Object=} bindings Properties to add to the controllers instance. This is used to simulate
 ...
 *
 * If you are using `$element` or `$attrs` in the controllers, make sure to provide them as `locals`.
 * The `$element` must be a jqLite-wrapped DOM element, and `$attrs` should be an object that
 * has all properties / functions that you are using in the controllers. If this is getting too complex,
 * you should compile the component instead and access the component's controllers via the
 * {@link angular.element#methods `controllers`} function.
 *
